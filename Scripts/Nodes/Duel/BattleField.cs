...
namespace MoDueler.Nodes {
    ...
    /// </summary
    [Obsolete("Battlefield should instead be created through lua.")]
    public class BattleField : Node2D {
        ...
        public void CreateSlots(bool flipped, Table mapping) {
            int fullField = (int)mapping["field"];
            Table subFields = (Table)mapping["subFields"];
            Table slots = (Table)mapping["fieldSlots"];
            Table columns = (Table)mapping["fieldColumns"];
        ...
        private void CreateZones(bool flipped, System.Collections.Generic.Dictionary<int, int> mapping) {
            ...
            Shader shader = GD.Load<Shader>("res://Shaders/BattleField.shader");
